...
const utils = {
    ...
    inlinePoints(initialX, initialY, endX = initialX, endY = initialY) {
        if (initialX !== endX) {
    ...
    checkSomeoneInFront(originX, originY, direction) {
        if (direction === "up") {
    ...
    checkCutsceneAtTile(originX, originY, direction) {
        if (direction === "up") {
            let middleX = originX + 8;
            let middleY = originY;
            return [`${middleX - 4},${middleY}`, `${middleX - 8},${middleY}`, `${middleX - 12},${middleY}`];
